...
while True:
    ...
    for d in digits:
        ...
        if d.size:
            ...
            d = to_model_tensor(d)
            pred = sm(cnn(d))
            pred_digit = pred.argmax().item() # The digit the model predicts
            prop = pred[0][pred_digit].item() # The probability of getting the digit the model predicts
            prop = round(prop, 4)
            if prop > 0.75:
                preds.append((pred_digit, prop))
                print(pred_digit, prop)
